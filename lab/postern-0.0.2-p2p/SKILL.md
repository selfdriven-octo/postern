...
src/enroll.js    CLI: keygen | pub | add | revoke | list   (single-responder registry)
src/roster.js    Roster: people -> devices -> grants; fingerprint; addresses
src/node.js      Node: one socket, both roles, merged sessions, keep-alives
src/roster-cli.js  CLI: init | add-device | revoke | grant | addr | list | fingerprint
src/peer.js      runs one mesh node; SIGHUP re-reads the roster
test.js          end-to-end suite: happy path plus every negative case
test-mesh.js     mesh suite: 8-node team, simultaneous open, revocation, restart
```
...
The reference operations are `ping`, `status`, `roster` and `echo`.
...
## Mesh mode (a team of peers)

`channel.js` is client-server by shape: a `Responder` binds and waits, an
`Initiator` calls. `node.js` is the symmetric peer — one socket that does both,
demultiplexing on the first byte, with one session table. Use it for a team
mesh; use the split classes when one side is genuinely a service.

`roster.js` replaces the flat `peers.json`. A peer is a **device**, not a
person, so a stolen laptop is revocable without touching that person's phone;
devices are grouped under a person so revoking someone is one edit.

**Two rules that are easy to get wrong:**

1. **Simultaneous open.** If both peers dial at once, both handshakes succeed and
   each side holds two sessions. Both sides must discard the same one with no
   negotiation, so the rule is local and deterministic: keep the session whose
   **initiator holds the higher static public key**.
2. **Restart beats the tie-break.** That rule applies only inside
   `RACE_WINDOW_MS` (1500 ms). A handshake arriving later is a peer that
   restarted and lost its table, and the **new session must win** — otherwise
   the pair deadlocks until the session TTL. `test-mesh.js` covers both.

**The fingerprint** covers keys, grants, revocation and version — deliberately
not addresses, so a roaming peer does not move the number the team reads aloud.
Routing-only edits (`roster-cli.js addr`) must not bump the version.

**Revocation has no authority behind it.** It takes effect per node as each node
updates; a node holding a stale roster still trusts a revoked key. The mesh
suite asserts this on purpose. Never paper over it — the `roster` op exists to
make disagreement visible, not to prevent it.

Run `node test-mesh.js` alongside `npm test` after any change to `node.js`,
`roster.js` or the session logic.
...
- [ ] `npm test` and `node test-mesh.js` both pass, including every negative case.
